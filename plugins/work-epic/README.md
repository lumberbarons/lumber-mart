...
One epic, pumped: the orchestrator never implements, reviews by hand, closes, or reconciles a
conflict itself. It accelerates everything up to the gates, not through them — and which gate
is which depends on the mode you pick.
...
and escalated; under `--resolve-conflicts` a reconciler agent instead merges the default
branch in and reconciles both sides' intent, and its new head is re-reviewed before any merge.
...
/work-epic:work-epic 12 --resolve-conflicts        # delegate PR conflicts to a reconciler agent
...
file-and-escalate a conflict — or, under `--resolve-conflicts`, hand it to a reconciler agent
and re-review the new head — hold on a blocking finding, flag branch protection it cannot
...
compare API, so this holds whether or not branch protection requires up-to-date branches. It
never merges outside the epic, never `--admin`, never force-pushes, and never reconciles a
conflict itself.

**The merge gate is CI + severity; the judgement gates stay human.** Closing issues and the
epic, clearing a hold, resolving a conflict (unless `--resolve-conflicts` delegates it), and
anything past a blocked worker's dialog are decisions the pump escalates, never makes.
